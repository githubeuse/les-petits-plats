...
function search() { //TODO: fonction de recherche
    ...
    if (formSearch.value.length > 2) {
        ...
        for (let singleDataRecord of dataRecord) { // pour chaque élément du tableau dataRecord
            ...
            let found = false; // Flag pour indiquer si on trouve une correspondance  

            if (labelLower.startsWith(searchLower)) {
                found = true;
                break; // Sort de la boucle si une correspondance est trouvée
            }
            if (found) {  // si une correspondance est trouvée et found passe à true (flag)
